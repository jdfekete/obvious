...
Weka~\cite{Weka} \new{and RapidMiner~\cite{RapidMiner} are} suites of
...
supports two mechanisms to build the main data structure of Weka \new{and
RapidMiner} from an Obvious Table:
\begin{itemize}[noitemsep,topsep=0pt]
\item an Obvious table can be copied into a Weka \new{or RapidMiner} data
...
\item an Obvious table can be wrapped as a Weka \new{or a
...
applied to the same machine learning algorithms from Weka \new{and
...
additional features, such as Weka \new{and RapidMiner} for advanced
machine-learning capabilities and several format converters.
\new{Furthermore, Weka and RapidMiner provide rich extensions such as
...
EdiDuplicate is a \new{VA} system designed to detect and merge duplicated
...
presented in the next section.  \new{Cached tables can be used as an
...
\new{Richness comes from the already large choice of visualizations and
